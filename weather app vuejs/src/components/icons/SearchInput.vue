...
const handleSearch = () => {
  ...
  searchTerm.timeout = setTimeout(async () => {
    if (searchTerm.query !== '') {
        const res = await fetch(`http://api.weatherapi.com/v1/search.json?key=07b9b9ebb06f4d39845112349242506&q=${searchTerm.query}`);
...
const getWeather = async (id) => {
    const res = await fetch(`http://api.weatherapi.com/v1/forecast.json?key=07b9b9ebb06f4d39845112349242506&q=id:${id}&days=3&aqi=no&alerts=no`)
    const data = await res.json()
